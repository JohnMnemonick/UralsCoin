...
#define START_MASTERNODE_PAYMENTS 1504224000
#define START_MASTERNODE_PAYMENTS_TESTNET 1504224000
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache, bool fDryRun)//TODO--
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s URALS additional fees, %d delta bytes\n",
...
//Skip checking block #1 with premine
            ...
            if(pindex->GetBlockHash() == block.hashPrevBlock){
                ...
                if (!fIsInitialDownload && pindex != NULL)
                {
                    ...
                    if (winningNode && pindex->nHeight > 132000) {
                        ...
                        LogPrintf("CheckBlock() : /*Check YIIMP bug*/ Skipping masternode payment check - nHeight %d Hash %s\n", chainActive.Tip()->nHeight+1, block.GetHash().ToString().c_str());
                    }
